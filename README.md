...
5.8  第一次合并：初步完成评论、答案、文章、问题详情页面 (假数据)； 按照android编码格式规范命名方式与编码格式；并从Google Meterial Design图标库中选出图标，替换原有不规范图标。
5.26 第二次合并：进一步修改评论、答案、文章、问题详情页面，并增加model层，增加部分POST请求。
5.30 第三次合并：修改问题和文章、答案的下拉刷新方式，用fragment替换原有实现方式；添加点赞和收藏的事件绑定；添加查看问题指定工作室；进一步修改命名不规范的地方；增加部分POST请求。
6.6  第四次合并：修改上一次合并后发现的bug；增加GET和DELETE请求；回答问题设置权限。
6.16 第五次合并：替换标准的点赞收藏图标；增加回答的富文本编辑器；修改点赞和收藏的逻辑；修改部分网络请求
6.24 第六次合并：增加文章、问题、回答的富文本显示；修改部分网络请求。
